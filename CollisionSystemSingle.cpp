...
CollisionSystemSingle::CollisionSystemSingle(Player *player, Block *block1, Block *block2, int xsize, int ysize)
{
	mPlayer = player;
	//mBlock = block;
	mBlock1 = block1;
...
/* //TWO BLOCKS THAT PASS THROUGH EACH OTHER
	...
	*/
	...
	case Movement::Up:
		...
		if (mPlayer->getYPos() < 0) //WRAP AROUND PLAYER
		{
			mPlayer->setYPos(ySize);
		}

		if (mBlock1->getXPos() == mPlayer->getXPos() && mBlock1->getYPos() == mPlayer->getYPos()) //PUSH BLOCK
		{
			mBlock1->setYPos(mBlock1->getYPos() - 1);//BLOCK TRAVEL DISTANCE
			mPlayer->setCollisions(mPlayer->getCollisions() + 1);
		...
		if (mBlock2->getXPos() == mPlayer->getXPos() && mBlock2->getYPos() == mPlayer->getYPos()) //PUSH BLOCK
		{
			mBlock2->setYPos(mBlock2->getYPos() - 1);//BLOCK TRAVEL DISTANCE
			mPlayer->setCollisions(mPlayer->getCollisions() + 1);
